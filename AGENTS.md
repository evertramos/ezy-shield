...
| `internal/config/secret_leak_test.go` + `internal/ai/secret_leak_test.go` + `internal/notify/secret_leak_test.go` + `internal/enrich/secret_leak_test.go` | Tokens never appear in errors, logs, or request bodies; notifier transport errors never echo the secret-bearing URL; the MaxMind license key never reaches an error/log via `*url.Error` (§4 SECURITY-REVIEW) |
...
- Writing user docs (`docs/content/`) in changelog voice — user docs describe current behavior timelessly: no issue references, no dev-history narrative ("since #N", "before this fix"), no real hostnames. Issues cite docs, never the reverse. (Audit-style references — e.g. `data-flow.md` pointing at source files so readers can verify claims — are fine.)
