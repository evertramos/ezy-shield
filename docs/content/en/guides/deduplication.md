...
| `"notify_only"` | Score in observe band; no ban. Returned on every evaluation, but audited once per (IP, rule) per minute while the rule stays saturated — so the digest's "notify-only" total counts those rising edges, while the `ezyshield_actions_total{op="notify_only"}` metric counts every evaluation |
