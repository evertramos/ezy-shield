...
- `--evidence` — include raw log excerpts mentioning the IP, extracted on
  demand from the daemon's configured log sources (requires an IP). Excerpts
  are bounded (most recent window, 50 lines per source) and never persisted;
  file sources that rotated away, and journald/docker sources, degrade to an
  explanatory note instead of failing the report
