...
The multi-arch image `ghcr.io/evertramos/ezyshield` is signed with the same
keyless workflow identity as `checksums.txt`. The identical image is also
mirrored to Docker Hub — `docker pull evertramos/ezyshield` works without
pointing at ghcr.io; both registries receive the same manifest from one
build, and prereleases never move `latest` on either. The signature lives
in the registry next to the image, so cosign fetches everything it needs:

```bash
VERSION=v0.1.0   # the release you are verifying (image tags drop the "v")
cosign verify \
  --certificate-oidc-issuer "https://token.actions.githubusercontent.com" \
  --certificate-identity-regexp \
    '^https://github\.com/evertramos/ezy-shield/\.github/workflows/release\.yaml@refs/(tags/v[0-9][^ ]*|heads/(main|dev))$' \
...
A successful run lists the checks performed (signature, transparency log,
certificate identity) and prints the verified signature payload as JSON.
cosign resolves the tag to the image **digest** and verifies against that,
so a re-pushed or moved tag fails verification. For the strongest pinning,
pull and run by digest — `ghcr.io/evertramos/ezyshield@sha256:<digest>` —
using the digest the verify output reports.

The image also carries a per-platform **SPDX SBOM**, attached at build time
as a BuildKit attestation (generated by the syft scanner inside the release
build — the container-side counterpart of the `.spdx.json` release assets).
Inspect it with:
...
- Releases published **before** signing landed (early v0.1.0 release
  ...
  TLS to `github.com` only. Likewise, container images pushed before image
  signing landed carry neither a cosign signature nor an SBOM attestation —
  `cosign verify` fails on them by design.
