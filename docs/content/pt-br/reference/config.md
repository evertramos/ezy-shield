...
| `parser` | não | força um parser: `nginx` \| `ssh` \| `apache` \| `apache-error` \| `traefik` \| `caddy` \| `dovecot` (padrão: roteado automaticamente a partir da fonte). `apache` lê o log de **acesso** do Apache (formato combined, compartilhado com `nginx`); `apache-error` lê o **error_log** do Apache (`error.log` / `error_log`); `dovecot` lê linhas dos processos de login do Dovecot (`dovecot.log`, ou a unit journald `dovecot` — falhas de auth IMAP/POP3 viram `imap_auth_fail` e sondas sem credenciais `imap_probe`; em setups com `mail.log` compartilhado use a unit journald ou este override explícito). **Honrado apenas para coletores `file` e `docker`** — o `journald` o ignora e sempre roteia o parser a partir da unidade. |
