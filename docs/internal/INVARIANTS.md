...
**C3. A rule fires on exactly the events it claims.** Fixed: aggregator flush now uses the longest window (#610; harness `TestDetection_HourlyTierSurvivesFlush`). Consequences of restoring the hourly tier, found by the adversarial review of #618 and filed: consumed evidence re-firing the ladder after a 5-min ban expires (#621 — fixed: a strike resets the IP's in-memory evidence, harness `TestDecision_NoSecondStrikeOnSameEvidence`); per-IP retention is unbounded and a busy client costs hundreds of MB and most of a core (#622 — fixed: the raw sample keeps the newest `DefaultMaxSamples` events per IP and events beyond the cap are folded into per-minute per-kind counters, so a bucket is bounded by ≈ `DefaultMaxSamples` × event size (≈ 4–6 MB at 4096 × ~1 kB, measured 6 MB for 100 req/s × 1 h against 377 MB before) and `Aggregate` costs O(cap + minutes) instead of O(rate × horizon); `Kinds` stays exact except that overflow counters are minute-wide, so a window cutoff inside a minute counts up to one extra minute of *evicted* history — only for IPs past the cap; tests `TestAggregate_BusyIPRetentionIsBounded`, `TestBusyIP_FieldLevelHourlyRuleSeesCurrentTraffic`, `BenchmarkAggregate_BusyIP`); the benchmark runs long windows in memory with no flush and has no hour-scale legit scenario (#623); future-dated events block eviction (#624). Fixed with #622: `Sample` now keeps the newest 4096, so busy IPs see field-level sustained rules on their current traffic (E3-2; `TestAggregate_SampleKeepsNewestEvents`). Violations: drop-in overrides keep the old `rule:` counter history (E3-3); validation accepts duplicate names, unreachable thresholds and unknown kinds (E3-4).
