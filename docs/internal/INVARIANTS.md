...
**D3. The AI layer is consulted only when it can change the outcome and never bypasses policy.** Verified: AI verdicts pass the identical guards. Fixed (#648): the async queue keeps an IP pending until its analysis is done and enforces a per-IP cooldown (1 min) between analyses, so one burst is one provider call, not one per event (`TestAIAsync_InFlightIPNotReenqueued`, `TestAIAsync_PerIPCooldown`). Open: async path never arms the anti-lockout re-check (D3-1) and swallows `ErrRateLimited` (D3-2); budget-exhausted "rules-only" is invisible to `status`/`doctor` (D3-3).
