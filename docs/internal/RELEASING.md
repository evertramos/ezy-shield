...
| `ghcr.io/evertramos/ezyshield:<ver>` (+ `:latest` on stable) | Multi-arch container image (amd64+arm64) carrying **both** binaries on a `scratch` base, non-root by default. Pushed to **GHCR** — not attached as a GitHub release asset. Cosign-signed at its digest + SPDX SBOM attestation (#407). See [Container image](#container-image-ghcr). |
...
- **One image, both binaries, `scratch` base.** The final stage is
  ...
  libc). `ENTRYPOINT` is the main CLI.
- **Non-root by default** (uid/gid `65532`) — the main daemon needs no
  privileges; the image never bakes in a privileged default.
- **Host-firewall enforcement from this image is NOT supported** (#407).
  An earlier version of this doc suggested `--entrypoint
  /usr/local/bin/ezyshield-enforcer --cap-add NET_ADMIN`; that recipe was
  wrong twice over, verified empirically with `docker run` on a snapshot
  image:
  1. The enforcer applies rules by exec'ing the `nft` userspace binary (and
     uses `ss` for pre-ban socket teardown) — a `scratch` image has neither,
     nor a libc to bind-mount them against, so the enforcer exits 1 at init
     (`nft -f: exec: "nft": executable file not found in $PATH`) on Docker
     **and** Podman, whatever capabilities are granted.
  2. Even with `nft` present, Docker's `--cap-add NET_ADMIN` lands only in
     the *bounding* set: a non-root (`USER 65532`) entrypoint gets **no
     effective** CAP_NET_ADMIN under `docker run` (no ambient-caps flag;
     `COPY` drops file caps; `scratch` has no `RUN` for `setcap`). Podman
     does grant ambient caps to non-root, but blocker 1 still applies. And
     without `--network host` the container would mutate its own netns'
     nftables, not the host's — silently ban-ineffective.

  **Supported topology:** run the enforcer on the host (deb/rpm or
  `get.sh` — the systemd unit carries `AmbientCapabilities=CAP_NET_ADMIN`)
  and give a containerized daemon its socket with
  `-v /run/ezyshield-enforcer:/run/ezyshield-enforcer` (align the container
  user with the socket's 0660 group). Edge enforcement (Cloudflare) needs no
  enforcer at all. The enforcer binary stays in the image for version parity
  and the CI smoke probe (`--entrypoint /usr/local/bin/ezyshield-enforcer
  <img> --version`).
- **Compose example + user guide: descoped for now** (#407). A compose file
  implies a fully supported containerized topology; until an
  enforcer-capable image variant exists (a base with `nft` + root or file
  caps — file a new issue if demand shows up), host installs are the
  recommended path and a compose example would mostly showcase the
  unsupported case.
- **Supply chain** (#407): every pushed image is **cosign-signed (keyless)**
  at its manifest-list digest via goreleaser's `docker_signs:` — the same
  OIDC workflow identity that signs `checksums.txt` — and carries a
  per-platform **SPDX SBOM attestation** generated by BuildKit's syft
  scanner (`dockers_v2.sbom: true`; publish-only, snapshots neither push nor
  attest, so PR CI is unaffected). Verification commands:
...
docker run --rm --entrypoint /usr/local/bin/ezyshield-enforcer "$IMG" --version  # version probe only
```
