...
func TestParseVerdictJSON_Regression_Issue21(t *testing.T) {
	...
	in := "```json\n" +
		`{"results":[{"ip":"198.51.100.211","score":72,"category":"scanner","confidence":0.85,"reason":"webshell probing","suggest_ttl_seconds":600}]}` +
		"\n```"
	var got verdictSchema
	if err := parseVerdictJSON(in, &got); err != nil {
		t.Fatalf("regression: %v", err)
	}
	if len(got.Results) != 1 || got.Results[0].IP != "198.51.100.211" {
