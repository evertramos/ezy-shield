...
func FuzzNginxParser(f *testing.F) {
	...
	f.Add([]byte(`{"remote_addr":"10.0.0.1","request":"GET / HTTP/1.1","status":"200","body_bytes_sent":"0","http_user_agent":"t","http_x_forwarded_for":"203.0.113.9, 198.51.100.7:54321"}`))                          // X-Forwarded-For behind a trusted proxy (issue #612)
	f.Add([]byte(`{"remote_addr":"10.0.0.1","request":"GET / HTTP/1.1","status":"200","body_bytes_sent":"0","http_user_agent":"t","http_x_forwarded_for":"203.0.113.9, unknown, [2001:db8::7]:443, ::ffff:10.0.0.9"}`)) // X-Forwarded-For behind a trusted proxy (issue #612)
	...
	f.Fuzz(func(_ *testing.T, b []byte) {
		p := parser.NewNginxParser(fuzzDiscardLogger(), parser.NginxConfig{TrustedProxies: []netip.Prefix{netip.MustParsePrefix("10.0.0.0/8")}})
		line := sdk.RawLine{
