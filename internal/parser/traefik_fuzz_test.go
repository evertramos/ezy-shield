...
func FuzzTraefikParser(f *testing.F) {
	...
	f.Add([]byte(`{"ClientAddr":"10.0.0.5:1234","ClientHost":"10.0.0.5","RequestMethod":"GET","RequestPath":"/","RequestProtocol":"HTTP/1.1","DownstreamStatus":200,"DownstreamContentSize":0,"request_X-Forwarded-For":"203.0.113.9, _hidden, 198.51.100.7","request_User-Agent":"t"}`)) // X-Forwarded-For behind a trusted proxy (issue #612)
	...
	f.Fuzz(func(_ *testing.T, b []byte) {
		p := parser.NewTraefikParser(fuzzDiscardLogger(), parser.TraefikConfig{TrustedProxies: []netip.Prefix{netip.MustParsePrefix("10.0.0.0/8")}})
		line := sdk.RawLine{
