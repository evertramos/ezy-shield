...
# one-liner. This is exercised against a local mock of the GitHub API (no
# real network calls, deterministic, safe to run on every PR) so the
# behavior can't silently regress once a real stable release exists and
# nobody hits this path organically anymore.
#
...
class H(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path.startswith("/repos/evertramos/ezy-shield/releases/latest"):
            if mode == "200":
                self._json(200, {"tag_name": "v0.1.0", "assets": []})
            else:
...
# (modes: 404 | 200 | both-404, see MOCK_SERVER_PY), runs the real get.sh
# against it via EZYSHIELD_API_BASE_URL, tears the mock down, and leaves the
# result in the globals OUT (combined stdout+stderr) and RC (exit code).
run_against_mock() {
  local mode="$1"
  ...
  set +e
  OUT="$(EZYSHIELD_API_BASE_URL="http://127.0.0.1:${port}" sh "$GET_SH" 2>&1)"
  RC=$?
  set -e
...
echo
echo "▸ Scenario: regression guard — a real stable release still resolves normally"
run_against_mock 200
case "$OUT" in
  *"Installing EzyShield v0.1.0 "*) ok "resolved the stable tag and proceeded to install" ;;
